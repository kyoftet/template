...
// SetNillableMailAddress sets the "mail_address" field if the given value is not nil.
...
func (uu *UserUpdate) check() error {
	...
	if v, ok := uu.mutation.MailAddress(); ok {
		if err := user.MailAddressValidator(v); err != nil {
			return &ValidationError{Name: "mail_address", err: fmt.Errorf(`ent: validator failed for field "User.mail_address": %w`, err)}
		}
	}
	return nil
}
